...
def create_noisy_channel(name: str, length: float, fidelity: float):
    """
    Create quantum channel with depolarizing noise.
    """
    ...
    return QuantumChannel(
        name,
        length=length,
        models={"quantum_noise_model": noise_model, "delay_model": delay_model}
    )

def create_network(dealer_name: str, recipient_names: list, eve_target: str = None, fidelity: float = 1.0):
    """
    Create a quantum network for GHZ-based secret sharing.

    Args:
        dealer_name: Name of the dealer (e.g., "Alice")
        recipient_names: List of recipient names (e.g., ["Bob", "Charlie"])
        eve_target: Name of recipient Eve intercepts (None = no Eve)

    Returns:
        nodes: Dict mapping names to Node objects
        eve_node: Eve's node (or None if no Eve)
    """
    nodes = {}
...
def _setup_ports(nodes, dealer_name, recipient_names, eve_node, eve_target):
    ...
    for recipient in recipient_names:
        ...
        # Add ports for communication with other recipients
        for other in recipient_names:
            if other != recipient:
                ports.extend([f"c_port_to{other}", f"c_port_from{other}"])
        # Add Eve port if this recipient is Eve's target
...
def _setup_channels(nodes, dealer_name, recipient_names, eve_node, eve_target, fidelity):
    """Setup quantum and classical channels."""
    dealer = nodes[dealer_name]
    ...
    for recipient in recipient_names:
        ...
        if eve_target == recipient and eve_node:
            # Route through Eve: Dealer -> Eve -> Recipient
            ...
            # Direct channel: Dealer -> Recipient
        ...
        # Classical channels: Dealer <-> Recipient
    ...
    # Classical channels between recipients
...
def reset_network(nodes, eve_node=None):
    """Reset all quantum memories in the network."""
